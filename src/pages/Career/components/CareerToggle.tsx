...
export const HideAnswerToggle = ({
  title,
  onToggle,
}: HideAnswerToggleProps) => {
  return (
...
export const ShowAnswerToggle = ({
  ...
  onUpdate,
  onDeleteSuccess,
}: ShowAnswerToggleProps) => {
  const handleDelete = async () => {
    try {
      await axios.delete(
        `${BASE_URL}/careers/special-skills?skillId=${skillId}`,
      );
      ...
      <div className="flex w-full flex-col items-start gap-[20px] p-[20px]">
        <div className="flex w-full flex-col items-start gap-[16px]">
          <textarea
            ref={titleRef}
            rows={1}
            className="font-regular w-full resize-none overflow-hidden rounded-sm border border-neutral-80 px-[20px] py-[14px] text-xsmall16 tracking-[-0.096px] text-neutral-40"
            value={newTitle}
            onChange={(e) => {
          ...
          <textarea
            ref={contentRef}
            className="font-regular w-full resize-none overflow-hidden rounded-sm border border-neutral-80 px-[20px] py-[14px] text-xsmall14 tracking-[-0.21px] text-neutral-40"
            value={newContent}
            onChange={(e) => {
