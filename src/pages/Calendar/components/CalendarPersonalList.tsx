...
/* ToDo 관련 Tools 임포트 */
import { PersonalScheduleListProps, formatDate1 } from "../../../components/ToDoListTool.ts"
...
// 개인 일정 API 연동
import { getPersonalSchedules, addPersonalSchedule } from '../../Main/api/personalApiService.ts';
import { PersonalSchedule, PostPersonalScheduleRequest } from '../../Main/api/personalType.ts';
...
const CalendarPersonalList = ({userId, userName, selectedDate, setSelectedDate} : CombinedProps) => {
    ...
    const [filteredSchedules, setFilteredSchedules] = useState<PersonalSchedule[]>([]); // 필터링된 일정
    ...
    const fetchSchedules = async () => {
        try {
            const response = await getPersonalSchedules({ userId: userId, year, month });
