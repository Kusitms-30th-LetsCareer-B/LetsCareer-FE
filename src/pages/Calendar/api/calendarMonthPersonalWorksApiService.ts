...
// API 연동 타입
import {
  CalendarMonthPersonalWorks_URL,
  GetParamsCalendarMonthPersonalWorksType,
  PostCalendarMonthPersonalWorksType,
} from "./calendarMonthPersonalWorksType.ts";

/** 조회 */
// GET 요청 및 응답받기 (백엔드와 API 연동)
export const getResponseCalendarMonthPersonalWorksList = async ({
  userId,
  year,
  month,
}: GetParamsCalendarMonthPersonalWorksType) => {
  try {
    const response = await AxiosInstance.get(
      `${BASE_URL}${CalendarMonthPersonalWorks_URL}`,
      {
        // 쿼리 파라미터 전달
    ...
    console.error(
      `캘린더 ${year}년 ${month}월 투두 리스트를 가져오는 중 오류 발생:`,
      error,
    );
...
export const postResponseCalendarPersonalWorksList = async ({
  userId,
  date,
  content,
}: PostCalendarMonthPersonalWorksType) => {
