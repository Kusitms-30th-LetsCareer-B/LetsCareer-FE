...
const CustomCalendar: React.FC<CalendarComponentProps> = ({
  ...
  // 기업 채용 일정 원본 데이터
  const [recruitmentsDataList, setRecruitmentsDataList] = useState<
    GetRequestCalendarMonthRecruitmentsType[]
  >([]);
  ...
  // 개인 일정 원본 데이터
  const [personalWorksDataList, setPersonalWorksDataList] = useState<
    GetRequestCalendarMonthPersonalWorksType[]
  >([]);
    ...
    if (userId && currentDate) {
      ...
      const fetchAllCalendarScheduleList = async () => {
        try {
          ...
          // 기업 채용 일정 요청 및 응답받기
          const responseRecruitments =
            await getResponseCalendarMonthRecruitmentsList({
              userId,
              year,
              month,
            });
          ...
          // 개인 일정 요청 및 응답받기
          const responsePersonalWorks =
            await getResponseCalendarMonthPersonalWorksList({
              userId,
              year,
              month,
            });
          ...
          /** 1. 기업 채용 일정 파싱 */
          // 필터별 기업 채용 일정 개수 계산
          const documentItems = responseRecruitments.data.filter(
            (item) =>
              item.filter === filterState.START ||
              filterState.FINISH ||
              filterState.WRITTEN,
          ); // "서류"
          const interviewItems = responseRecruitments.data.filter(
            (item) => item.filter === filterState.INTERVIEW,
          ); // "면접"
          const otherItems = responseRecruitments.data.filter(
            (item) => item.filter === filterState.OTHER,
          ); // 기타 일정
  ...
  const handleDateSelect = (date: Date) => {
    ...
    const recruitmentScheduleChips = recruitmentsSchedulesForDate.map(
      (recruitmentsData) => {
        // 서류 칩
        const documentStatus =
          recruitmentsData.filter === filterState.START
            ? "시작"
            : recruitmentsData.filter === filterState.FINISH
              ? "마감"
              : "";
        ...
        // 일정 종류에 맞는 칩을 반환
        return recruitmentsData.filter === filterState.INTERVIEW
          ? interviewScheduleChip
    ...
    const personalScheduleChips = personalSchedulesForDate.map(
      ...
      */}
          <PersonalScheduleChip
            key={personalScheduleData.personalScheduleId}
    ...
    <div className="bg-neutral-100 p-4 font-sans shadow-none">
      ...
      <div className="grid grid-cols-7 gap-0">
        ...
        {dates.map((date, index) => {
          ...
          const recruitmentsSchedulesForDate =
            getRecruitmentsSchedulesForDate(date);

          {
            /** CalendarChips를 셀에 추가:  API 연동받은 정보를 참고하여 선택된 날짜에 대해 기업 일정 칩스 추가 */
          }
          const companySchedules = recruitmentsSchedulesForDate.map(
            (recruitmentsData) => {
              // 서류 칩스
              const documentScheduleChip =
                // 셀 선택 상태
          ...
          const personalSchedules = personalSchedulesForDate.map(
            (personalScheduleForDate) => {
              // 개인 일정칩스
