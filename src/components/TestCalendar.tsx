...
const CalendarComponent: React.FC<CalendarComponentProps> = ({ userId, year, month, selectedDate }) => {
  // company별로 채용 일정 데이터를 담은 list
  const [companyRecruitmentsList, setCompanyRecruitmentsList] = useState<GroupedByCompany>({});
  ...
  // 컴포넌트가 렌더링될 때 API 호출
  useEffect(() => {
    if (userId && year && month) {  // userId가 있어야(로그인 상태여야) 작동되니깐 검증용으로.
      const fetchCompanyRecruitmentList = async () => {
        try {
          ...
          const selectedItems = response.data.filter((data: GetRequestCalendarMonthRecruitmentsType) => {
          ...
          const groupedByCompany = selectedItems.reduce((acc: GroupedByCompany, schedule: GetRequestCalendarMonthRecruitmentsType) => {
  ...
  return (
    <div>
      <h2>{year}년 {month}월 채용 일정</h2>
      {Object.keys(companyRecruitmentsList).length > 0 ? (
        Object.entries(companyRecruitmentsList).map(([companyName, companyDataFrame]) => (
          <div key={companyName}>
            <h3>{companyName}</h3>
            <ul>
              {String(companyDataFrame)}
              {companyDataFrame.map((companyData: GetRequestCalendarMonthRecruitmentsType) => (
                <li key={companyData.scheduleId}>
                  {String(companyData.companyName)}
                  {String(companyData.date)}
                  <strong>{new Date(companyData.date).toLocaleDateString()}</strong>: {companyData.companyName} - {companyData.filter}
                </li>
              ))}
            </ul>
          </div>
