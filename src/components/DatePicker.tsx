...
const CustomCalendar = ({ onCancel, onSelect, message }: DatePickerProps) => {
  ...
  return (
    /* 캘린더 전체 윤곽 컨테이너 스타일 */
    <div className="w-[360px] rounded-lg border border-neutral-90 font-sans shadow-xl">
      {" "}
      ...
      <div>
        ...
        {/* 오늘 날짜 출력 */}
        {/* 선택된 날짜를 출력하고 싶으면 (new Date) 대신에 selectedDate 넣기 */}
        <div className="mb-6 px-5 text-medium24 font-semibold text-static-0">
          {new Date().getMonth() + 1}월 {new Date().getDate()}일 {`(`}
          {daysOfWeekKor[new Date().getDay()]}
          {`)`}
        </div>
      ...
      <div className="grid grid-cols-7 gap-0">
        ...
        {dates.map((date, index) => {
          ...
          const dateContent = date ? (
            ...
            <span
              className={`flex h-full w-full items-center justify-center ${isSelected ? "border-5 h-10 w-10 rounded-full border-primary-100 bg-primary-100 text-white" : ""} ${isHovered ? "border-5 h-10 w-10 rounded-full border-primary-100 bg-primary-100 text-white" : ""}`}
            >
              {/* 날짜 셀에 들어갈 데이터 */}
              {date.getDate()}
            </span>
      ...
      <div className="flex justify-end px-5 py-3">
        ...
        <div
          onClick={onCancel}
          className="rounded-lg px-7 text-xsmall14 text-neutral-40 hover:text-primary-100"
        >
          취소
        </div>
        {/* 선택완료 버튼 */}
        <div
          ...
          className="rounded-lg text-xsmall14 text-neutral-40 hover:text-primary-100"
        >
          선택완료
        </div>
        <br />
